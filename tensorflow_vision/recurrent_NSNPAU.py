...
class NSNPAU(Recurrent):
    ...
    def __init__(self, units,
                 ...
                 recurrent_dropout=0.,
                 **kwargs):
        super(NSNPAU, self).__init__(**kwargs)
        self.units = units
        self.activation = activations.get(activation)
    ...
    def build(self, input_shape):
        ...
        if self.use_bias:
            ...
            self.bias_a = None
        self.built = True
        self.reset_gate_peephole_weights = self.add_weight(shape=(self.units,),
                                                           name='reset_gate_peephole_weights',
                                                           initializer=self.kernel_initializer)
        self.consume_gate_peephole_weights = self.add_weight(shape=(self.units,),
                                                            name='consume_gate_peephole_weights',
                                                            initializer=self.kernel_initializer)
        self.output_gate_peephole_weights = self.add_weight(shape=(self.units,),
    ...
    def step(self, inputs, states):
        ...
        if self.implementation == 2:
            ...
            r = self.recurrent_activation(z0 + self.reset_gate_peephole_weights * u_tm1)
            c = self.recurrent_activation(z1 + self.consume_gate_peephole_weights * u_tm1)
            o = self.recurrent_activation(z2 + self.output_gate_peephole_weights * u_tm1)
            ...
            r = self.recurrent_activation(x_r + K.dot(h_tm1 * rec_dp_mask[0],
                                                      self.recurrent_kernel_r) + self.reset_gate_peephole_weights * u_tm1)
            c = self.recurrent_activation(x_c + K.dot(h_tm1 * rec_dp_mask[1],
                                                      self.recurrent_kernel_c) + self.consume_gate_peephole_weights * u_tm1)
            o = self.recurrent_activation(x_o + K.dot(h_tm1 * rec_dp_mask[2],
                                                      self.recurrent_kernel_o) + self.output_gate_peephole_weights*u_tm1)
            a = self.activation(x_a + K.dot(h_tm1 * rec_dp_mask[3],
    ...
    def get_config(self):
        config = {'units': self.units,
                  ...
                  'dropout': self.dropout,
                  'recurrent_dropout': self.recurrent_dropout}
        base_config = super(NSNPAU, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
