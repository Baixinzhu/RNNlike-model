...
# @tf_export(v1=["nn.rnn_cell.GSNP"])
class GSNP(LayerRNNCell):
  ...
  def build(self, inputs_shape):
    ...
    self._a_kernel = self.add_variable(
        "candidate/%s" % _WEIGHTS_VARIABLE_NAME,
        shape=[input_depth + self._num_units, self._num_units],
        initializer=self._kernel_initializer)
    self._a_bias = self.add_variable(
  ...
  def call(self, inputs, state):
    ...
    u_tm1 = state
    r, c = array_ops.split(value=value, num_or_size_splits=2, axis=1)

    # r_state = r * state

    a = math_ops.matmul(
        array_ops.concat([inputs, u_tm1], 1), self._a_kernel)
    a = nn_ops.bias_add(a, self._a_bias)
  ...
  def get_config(self):
    ...
    base_config = super(GSNP, self).get_config()
    return dict(list(base_config.items()) + list(config.items()))
...
class EGSNPCell(LayerRNNCell):
  ...
  def build(self, inputs_shape):
    ...
    if self.dtype is None:
      ...
      initializer = init_ops.zeros_initializer(dtype=self.dtype)
    one_initializer = init_ops.ones_initializer
    self._bias = self.add_variable(
        _BIAS_VARIABLE_NAME,
        shape=[4 * self._num_units],
    ...
    self._a_bias = self.add_variable(
        ...
        shape=[self._num_units],
        initializer=initializer)
    self._r_prev_kernel = self.add_variable(
        "r_prev_kernel", shape=[2 * self._num_units, self._num_units], initializer=self._initializer)
    self._c_prev_kernel = self.add_variable(
        "c_prev_kernel", shape=[2 * self._num_units, self._num_units], initializer=self._initializer)
  ...
  def call(self, inputs, state):
    ...
    if self._state_is_tuple:
      ...
      u_prev = array_ops.slice(state, [0, 0], [-1, self._num_units])
    ...
    lstm_matrix = math_ops.matmul(
        array_ops.concat([inputs, h_prev, u_prev], 1), self._kernel)
    lstm_matrix = nn_ops.bias_add(lstm_matrix, self._bias)
    lstm_matrix = sigmoid(lstm_matrix)
    r,c,i,j = array_ops.split(
        value=lstm_matrix, num_or_size_splits=4, axis=1)
    # Diagonal connections
    a = math_ops.matmul(
        array_ops.concat([inputs, u_prev], 1), self._a_kernel)
    a = nn_ops.bias_add(a, self._a_bias)
    ...
    new_state = (
        LSTMStateTuple(u, h)
        if self._state_is_tuple else array_ops.concat([u, h], 1))
    return h, new_state
  ...
  def get_config(self):
    ...
    base_config = super(EGSNPCell, self).get_config()
    return dict(list(base_config.items()) + list(config.items()))
